...
process_execute (const char *file_name) 
{
  ...
  tid_t tid;
  token = (char*)malloc(strlen(file_name)+1);
...
start_process (void *file_name_)
{
  ...
  //parse = realloc(parse,(count+1)*sizeof(char*));
  parse[count] = token;
  count++;
  }
...
void argument_stack(char **parse, int count, void **esp){
  ...
  *esp = *esp - ((count+1)*4);
  memset(*esp+(count*4),0,4);
  for(i = count-1; i>=0; i--){
    memcpy(*esp+i*4,(char*)(&address[i]),4);
  }
